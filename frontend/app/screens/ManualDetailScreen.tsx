 ...
 useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      if (!idPost) return Alert.alert("Error", "No se encontró el ID del manual");
      setPostId(Number(idPost));

      await AsyncStorage.setItem("postId", String(idPost));

      const manualData = await getPostById(Number(idPost));
...
const handleSubmitComment = async () => {
  ...
  if (!userId || !postId) return Alert.alert("Error", "Falta el ID de usuario o del manual");
