        ...
        async registerCompany(request: Request, response: Response): Promise <Response>{
            ...
            try{
                ...
                const company: Omit<Companies, "company_id"> = {
                    bussines_name, profession, document_number, doc_type_id, email, password, years_experience, role_id: 2, status: 1, created_at: new Date()
                };
                const user_id = await this.app.createCompany(company);
    
                return response.status(201).json({message:"Empresa creada exitosamente:", user_id});
            }catch(error){
        ...
        async updateCompany(request: Request, response: Response): Promise<Response>{
            try{
                ...
                if (profession && !Validators.name(profession)) 
                    return response.status(400).json({message:"La profesión debe tener al menos 3 caracteresponse y solo contener letras",
                });
          ...
          const updated = await this.app.updateCompany(company_id,{ bussines_name, document_number, profession, email, password, doc_type_id, years_experience, status: 1});
          if(!updated) return response.status(404).json({message: "Empresa no encontrada o sin cambios"});
    
          return response.status(200).json({message:"Empresa actualizada exitosamente"})
