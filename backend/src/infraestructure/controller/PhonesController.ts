...
export class PhonesController {
  ...
  async registerPhone(request: Request, response: Response): Promise<Response> {
    ...
    try {
      const phone: Omit<Phones, "phone_id"> = {
        number,
        number_type: {
          id: number_type_id,
          description: "" 
      ...
      return response
        .status(201)
        .json({ message: "Teléfono creado exitosamente", phoneId });
    } catch (error) {
  ...
  async downPhone(request: Request, response: Response): Promise<Response> {
    try {
      ...
      return response
        .status(200)
        .json({ message: "Teléfono eliminado exitosamente" });
    } catch (error) {
      if (error instanceof Error) {
        return response.status(500).json({ message: "Error en el servidor" });
      }
  ...
  async updatePhone(request: Request, response: Response): Promise<Response> {
    try {
      ...
      const { number, number_type_id } = request.body;

      const updated = await this.app.updatePhone(phoneId, {
        number,
        number_type: {
          id: number_type_id,
          description: "" // solo se actualiza con el id
        }
      });
      ...
      if (!updated)
        return response
          .status(404)
          .json({ message: "Teléfono no encontrado o sin cambios" });

      return response
        .status(200)
        .json({ message: "Teléfono actualizado exitosamente" });
    } catch (error) {
      if (error instanceof Error) {
        return response.status(500).json({ message: "Error en el servidor" });
      }
